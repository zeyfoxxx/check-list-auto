...
TEMPLATE_PATH = r"C:\Users\lbenadyext\Desktop\check-list-auto-main\check-list auto\CD13_MQ_.xlsx"
OUTPUT_DIR = r"C:\Users\lbenadyext\Desktop\check-list-auto-main\check-list auto"
TEMP_DIR = r"C:\Users\lbenadyext\Desktop\check-list-auto-main\check-list auto\check_temp" 
...
nouveau_fichier = os.path.join(OUTPUT_DIR, f"CD13_MQ_{date_aujourdhui}.xlsx")
            ...
            try:
                ...
                for att in msg.Attachments:
                    ...
                    if "prtg" in nom_att and nom_att.endswith('.csv'):
                        att.SaveAsFile(os.path.join(TEMP_DIR, "prtg_sites.csv"))
                    ...
                    # Si c'est le fichier METEO
                        ...
                        att.SaveAsFile(os.path.join(TEMP_DIR, "meteocd13.csv"))
                        print(f"-> CSV Météo récupéré : {att.FileName}")

                    elif "onepage" in nom_att and nom_att.endswith('.pdf'):
                        att.SaveAsFile(os.path.join(OUTPUT_DIR, "Top_Applications_"+date_aujourdhui+".pdf"))
                ...
                if any(word in sujet for word in ["check","image", "prtg", "météo","panorama"]):
                    ...
                    for att in msg.Attachments:
                        if att.FileName.lower().endswith(('.png', '.jpg', '.jpeg')):
                            ...
                            att.SaveAsFile(os.path.join(TEMP_DIR, nom_img))
                            compteur += 1
                    if compteur > 0:
...
def generer_excel():
    ...
    try:
        if not os.path.exists(TEMPLATE_PATH): # Je lui demande de vérifier si le template est bien trouvable dans le chemin donnée
            print("ERREUR : Le fichier Template est introuvable !")
            return

        wb = load_workbook(TEMPLATE_PATH) # Si il y a bien le template je le fais charger
        ws = wb.active

        #  rajout des images dans les casses du tableau E12 à E15
        for i in range(4):
            chemin_img = os.path.join(TEMP_DIR, f"photo_{i}.jpg")
        ...
        path_prtg = os.path.join(TEMP_DIR, "prtg_sites.csv")
        ...
        path_meteo = os.path.join(TEMP_DIR, "meteocd13.csv")
        ...
        chaine_bug2 = "274, 176, 153, 164, 164, 164"

        if ws['E8'].value == chaine_bug2  :
            ws['E8'] = "Ø"
            print(f"remplacement des faux positif du dossier météoCD13")   
